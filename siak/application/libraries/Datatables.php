  ...
  class Datatables
  {
    ...
    */
    public function select($columns, $backtick_protect = TRUE)
    {
      ...
      $this->ci->DB1->select($columns, $backtick_protect);
      return $this;
    }
    ...
    */
    public function join($table, $fk, $type = NULL)
    {
      ...
      $this->ci->DB1->join($table, $fk, $type);
      return $this;
    }
    ...
    */
    public function where($key_condition, $val = NULL, $backtick_protect = TRUE)
    {
      ...
      $this->ci->DB1->where($key_condition, $val, $backtick_protect);
      return $this;
    }
    ...
    */
    public function or_where($key_condition, $val = NULL, $backtick_protect = TRUE)
    {
      ...
      $this->ci->DB1->or_where($key_condition, $val, $backtick_protect);
      return $this;
    }
    ...
    */
    public function where_in($key_condition, $val = NULL)
    {
      ...
      $this->ci->DB1->where_in($key_condition, $val);
      return $this;
    }
    ...
    */
    public function like($key_condition, $val = NULL, $side = 'both')
    {
      ...
      $this->ci->DB1->like($key_condition, $val, $side);
      return $this;
    }
    ...
    */
    public function or_like($key_condition, $val = NULL, $side = 'both')
    {
      ...
      $this->ci->DB1->or_like($key_condition, $val, $side);
      return $this;
    }
    ...
    */
    private function get_paging()
    {
      ...
      if($iLength != '' && $iLength != '-1')
        $this->ci->DB1->limit($iLength, ($iStart)? $iStart : 0);
    }
    ...
    private function get_ordering()
    {
      ...
      if ($this->ci->input->post('order'))
        foreach ($this->ci->input->post('order') as $key)
          if($this->check_cType())
            $this->ci->DB1->order_by($Data[$key['column']]['data'], $key['dir']);
          else
            $this->ci->DB1->order_by($this->columns[$key['column']] , $key['dir']);
    ...
    */
    private function get_filtering()
    {
      ...
      $sSearch = $this->ci->DB1->escape_like_str(trim($search['value']));
      ...
      if($sWhere != '')
        $this->ci->DB1->where('(' . $sWhere . ')');

      // TODO : sRangeSeparator

      foreach($this->filter as $val)
        $this->ci->DB1->where($val[0], $val[1], $val[2]);
    }
    ...
    */
    ...
    {
      return $this->ci->DB1->get($this->table);
    }
    ...
    */
    private function get_total_results($filtering = FALSE)
    {
      ...
      foreach($this->joins as $val)
        $this->ci->DB1->join($val[0], $val[1], $val[2]);

      foreach($this->where as $val)
        $this->ci->DB1->where($val[0], $val[1], $val[2]);

      foreach($this->or_where as $val)
        $this->ci->DB1->or_where($val[0], $val[1], $val[2]);
        
      foreach($this->where_in as $val)
        $this->ci->DB1->where_in($val[0], $val[1]);

      foreach($this->group_by as $val)
        $this->ci->DB1->group_by($val);

      foreach($this->like as $val)
        $this->ci->DB1->like($val[0], $val[1], $val[2]);

      foreach($this->or_like as $val)
        $this->ci->DB1->or_like($val[0], $val[1], $val[2]);

      if(strlen($this->distinct) > 0)
      {
        $this->ci->DB1->distinct($this->distinct);
        $this->ci->DB1->select($this->columns);
      }
      $query = $this->ci->DB1->get($this->table, NULL, NULL, FALSE);
      return $query->num_rows();
      // $subquery = $this->ci->DB1->get_compiled_select($this->table);
      // $countingsql = "SELECT COUNT(*) FROM (" . $subquery . ") SqueryAux";
      // $query = $this->ci->DB1->query($countingsql);
