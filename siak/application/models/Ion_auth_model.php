...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($this->identity_column !== 'email') {
			$this->db->select($this->identity_column . ', email, id, password, active, last_login')
					 ->where($this->identity_column, $identity)
					 ->or_where('email', $identity);
		}else{
			$this->db->select('username, '.$this->identity_column.', id, password, active, last_login')
					 ->where('username', $identity)
					 ->or_where($this->identity_column, $identity);
		}
		
		$query = $this->db->limit(1)->order_by('id', 'desc')->get($this->tables['users']);
